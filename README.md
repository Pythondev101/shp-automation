...
### Required fields (tests 22–32, `functional`; added 2026-09-25)

**Why the old save tests broke.** The application now marks most profile fields as required, and Save Profile checks them in the browser before it sends anything. The account's saved profile has **City, Pincode, Address 1 and Address 2 empty**, so pressing Save shows the toast "Please select a city." and sends no request. Tests 16 and 20 were waiting for the `POST /api/v1/profile` answer and timed out.

**Required fields in the live UI (2026-09-25).** A required field's label ends with a red `<span class="text-danger"> *</span>`. The controls have **no** `required` attribute, and an invalid field gets no error style. `MyProfilePage.required_fields()` reads the marked labels from the page (the Security card's password fields are left out) instead of using a fixed list:
- Editable: Full Name, Company Name, Country, State, City, Timezone, Pincode, Address 1, Address 2.
- Read-only: Username (disabled) and "Email (change via OTP)" (readonly). They are not edited; test 22 only checks they have a value.
- **Mobile is not marked required** in the live UI (its label is "Mobile (change via OTP)"), although the screenshot showed it with `*`.
- State is marked only after the country's states have loaded, so `wait_until_loaded()` runs first.

**Validation.** Empty fields are checked one at a time, in form order, and only the first one is reported, as a toast (role `status`). The messages are kept in `REQUIRED_FIELD_MESSAGES`: "Full name is required.", "Company name is required.", "Please select a country.", "Please select a state.", "Please select a city.", "Please select a timezone.", "Pincode is required.", "Address 1 is required.", "Address 2 is required.". Clearing State also empties City and disables it.

| # | Test | Verifies |
|---|---|---|
| 22 | `test_required_fields_are_marked` | The live page marks all 9 editable fields above as required, and no other editable field (a new one fails the test, because its message is unknown). Username and Email are marked required, read-only and not empty |
| 23–31 | `test_save_profile_rejects_empty_required_field[<field>]` | One test per required editable field (parametrized). Every required field is filled with valid values (not saved). Then that one field is emptied (a text field is cleared, a dropdown set to "Select") and Save Profile is pressed. Verifies that field's toast appears, **no** `POST /api/v1/profile` is sent, and after a reload the profile still matches the values captured at the start. Sends no save, so it runs even while the saved profile is incomplete |
| 32 | `test_save_profile_with_all_required_fields_and_restore` | Captures the original values of the 9 fields (`profile_values()`). Fills every one with valid values from the live options (`fill_profile()`). Checks that no required field is still empty, then saves. Verifies `status: true`, the toast "Profile updated successfully.", and after a reload that every value persisted. The reload also waits for the states and cities the application loads for the saved country and state: the State list must be exactly that country's states and the City list exactly that state's cities, and the saved state and city must be among them. It then fills the originals back, saves again (`status: true` required, otherwise `PROFILE NOT RESTORED` with the original values), and a final reload must match the original values exactly. `destructive`, `shared_state` |

**Valid test data (`_valid_profile_values`).** Full Name `SHP Automation Test`, Company Name `SHP Automation Test Company`, Address 1/2 `101 Dummy Test Street` / `Dummy Test Town` (made-up addresses, not real data). Country stays the saved one, because the Pincode example depends on it. State is another state of that country with cities (up to 10 are tried). City is that state's first city, after checking that the City list is exactly the cities returned for that state. Timezone is any other than the saved one. Pincode is the application's own example from the placeholder. No country, state or city is hard-coded.

**Country → State → City.** `fill_profile()` always fills Country first, then State, then City. A changed country waits for `GET …/states`, and a changed state waits for `GET …/cities`. An unchanged one sends no request, so it is not selected again. The restore uses the same order, so the original state's cities are loaded before the original city is chosen.

**Precondition for the saving tests.** The application refuses to save an empty required field, so a required field that is empty in the saved profile can never be put back to empty through the UI. Tests 16, 20 and 32 therefore need every required field of the saved profile to hold a value. A `_skip_unless_required_fields_saved()` guard was first added to skip them while one was empty. On 2026-09-25 the user filled City, Pincode, Address 1 and Address 2 by hand and asked for the guard to be removed, so these tests now always run.

New code: in `my_profile_locators.py`, `REQUIRED_MARK`, `REQUIRED_FIELD_MESSAGES`, `required_labels` and `control_of()`. In `MyProfilePage`, `PROFILE_FIELDS`, `wait_until_loaded()`, `required_fields()`, `empty_required_fields()`, `profile_values()`, `fill_profile()`, `reload_reading_lists()` and `press_save_expecting_validation()` (records any save request sent while the validation toast appears). Files modified: `framework/locators/my_profile_locators.py`, `framework/pages/my_profile_page.py`, `tests/ui/test_my_profile.py`, and this section.

**Runs 2026-09-25 (Chromium, password test 21 not run).** First run, before the profile was completed: tests 22–31 passed, and 16, 20 and 32 skipped. An earlier combined run hit fixture timeouts opening `/dashboard` (`Page.goto` > 60 s, the slow server; `NAVIGATION_TIMEOUT_MS` in `tests/conftest.py` is unchanged), and 1–15 and 17–19 then passed 18/18 on re-run. After the profile was completed and the guard removed: 16, 17, 20, 22 and 32 passed 5/5. A read-back afterwards showed the profile exactly as before the run.
...
- **Client-side checks.** (2026-09-14; since superseded, see "Required fields" above.) Save Profile checked only that Full Name is not empty, that a country is selected, and the pincode format of that country. Full Name has `maxlength="30"` and **no client-side numeric check**, so a numeric-only name is sent and rejected by the server. Address 1 and Address 2 have `maxlength="50"`.
...
| 5–13 | `test_table_header_is_visible[<header>]` | S.No, Status, Order ID, Order Status, Total, Buyer, Qty, Part #, Order Date. **Fix (2026-09-25):** every header but S.No became sortable and now ends with a sort glyph (`Order ID⇅`, `Order Date▼` on the default sorted column). The table itself is found by its "Order ID" header, and that exact-name match no longer found it, so every table test failed. `_header_name()` in `order_processing_locators.py` now allows a trailing glyph (`^<name>\W*$`), both for the table and for `column_header()`. Order Tracking subclasses these locators, so the one fix covers it too. Runs 2026-09-25 (both files, Chromium): first 31 passed / 7 failed / 4 errors – the failures were my own slip (the helper was pasted above `column_cells()` / `calendar_day()`, which took them out of the class; fixed), the errors were setup timeouts on the slow server. Second run of the affected tests: 23 passed, 2 skipped (Order Status offers only "All"), 8 setup timeouts (`Page.goto` > 60 s, list request > 30 s), no test failures |
...
| 5–11 | `test_table_header_is_visible[<header>]` | S.No, Name, Setup Type, Setup Files, View Linking, Status, Action. Since 2026-09-25, Name, Setup Type, Setup Files and Status are sortable, and their header text ends with a `⇅` glyph (`Name⇅`). The old exact-name match stopped finding them, so `column_header()` now matches the name followed by any non-word glyph (`^<name>\W*$`). 12/12 passed on Chromium after the fix |
